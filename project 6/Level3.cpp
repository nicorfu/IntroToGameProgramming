...
void Level3::update(float delta_time, float curr_ticks)
{
	...
	if (m_state.portal->get_portal_touched())
	{
		...
		if (level_done)
		{
			m_state.player->set_player_won(true);
		}
	}
}
...
void Level3::render(ShaderProgram* g_shader_program)
{
	...
	if (m_state.player->get_player_won())
	{
		glm::vec3 text_position = glm::vec3(m_state.player->get_position().x - 1.9f, m_state.player->get_position().y + 2.0f, 0.0f);
		m_state.text->draw_text(g_shader_program, "YOU WON", 0.6f, 0.0001f, text_position, FONTBANK_SIZE);
	}
